...
<#
.SYNOPSIS
    Records optional DriFT startup telemetry through the PostTelemetryData Azure Function.

.DESCRIPTION
    Keeps DriFT from writing directly to Azure Table Storage. The Azure Function owns
    the table SAS token and writes only the Data object to DriFTTelemetryData.
#>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$true)]
    ...
    $script:uploadToAzure = -not [bool]$Context.NoTelemetry
    if (-not $script:uploadToAzure) { return }

    $telemetryParams = @{
        TelemetryName  = "DriFTTelemetryData"
        EventName      = "Startup"
        Version        = $($script:DriFTVersion -replace "DriFT_v")
